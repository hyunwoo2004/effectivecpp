...
// 구체적으로는 close을 호출해 줄 것
// 그 부분이 뭔가 잘못되었다고 하면(소멸자에서 호출된 close에서 예외를 던진다면) DBConn이 예외를 삼키든 프로그램을 종료하든 사용자쪽은 불평할 자격이 없음
// 아무튼 문제 해결의 칼자루를 먼저 쥔 쪽은 사용자이고, 칼을 뽑지 않기로 결정한 쪽도 역시 사용자임
